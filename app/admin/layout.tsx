...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted) {
      if (!isAuthenticated) {
        // 🔥 CORRECTION CRITIQUE : Si le front perd l'utilisateur, on force la destruction du cookie !
  ...
  // 🔥 ANTI PAGE BLANCHE : Au lieu de "return null", on affiche un feedback visuel
  if (getUserRole() !== 'ADMIN' || !isAuthenticated) {
    return (
      ...
      <div className="relative flex min-h-screen w-full overflow-hidden bg-background font-sans">
        ...
        <SidebarInset className="relative z-10 flex flex-col bg-transparent overflow-hidden w-full">
          <header className="flex h-16 shrink-0 items-center gap-2 border-b border-border/40 bg-card/60 backdrop-blur-xl px-6 md:px-8 shadow-sm">
            ...
            <span className="text-xs font-bold text-muted-foreground/70 uppercase tracking-widest">Espace Administration</span>
          </header>
