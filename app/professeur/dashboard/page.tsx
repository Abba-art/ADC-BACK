...
import { BookOpen, Building2, Clock, GraduationCap, ShieldAlert, CheckCircle, Hourglass, MapPin, Printer, Filter } from 'lucide-react';
...
export default function ProfesseurDashboard() {
  ...
  // État pour le filtrage par institut
  ...
  // --- EXTRACTION DES INSTITUTS POUR LE FILTRE ---
  ...
  if (isLoading) return <div className="p-8"><Skeleton className="h-20 w-[300px] mb-6 rounded-xl" /><div className="grid grid-cols-3 gap-6"><Skeleton className="h-40 rounded-2xl" /><Skeleton className="h-40 rounded-2xl" /><Skeleton className="h-40 rounded-2xl" /></div></div>;
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6 print:space-y-4">
      ...
      {/* EN TÊTE (Masqué à l'impression si besoin, mais agréable à garder) */}
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-muted-foreground mt-1">Bienvenue, <span className="font-bold text-foreground">{profil.nom} {profil.prenom}</span>. Voici le résumé de vos activités.</p>
        </div>
          ...
          <Card className="bg-card/60 backdrop-blur-2xl shadow-xl border-border/40 overflow-hidden h-full print:shadow-none print:border-border">
            ...
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                <Clock className="w-4 h-4 print:hidden" /> Charge Horaire
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <motion.div variants={itemVariants}>
          <Card className="bg-card/60 backdrop-blur-2xl shadow-xl border-border/40 overflow-hidden h-full print:shadow-none print:border-border">
            ...
            <CardContent>
              ...
              <p className="text-sm font-medium text-muted-foreground">Blocs d&apos;enseignement validés pour l&apos;année en cours.</p>
            </CardContent>
          </Card>
        </motion.div>
            ...
            <CardContent className="flex-1 flex flex-col justify-center">
              {Object.keys(repartitionInstituts).length === 0 ? (
                <div className="text-center text-muted-foreground">
                  <p className="text-sm">Aucune heure consommée.</p>
                </div>
              ) : (
        ...
        <Card className="bg-card/60 backdrop-blur-2xl shadow-xl overflow-hidden border-border/40 print:shadow-none print:border-none">
          <CardHeader className="border-b border-border/40 pb-4 pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:border-b-2">
            <div>
              ...
              <CardDescription className="print:hidden">Liste détaillée de vos affectations pédagogiques.</CardDescription>
            </div>
            
            {/* 🔥 LE FILTRE PAR INSTITUT (Masqué à l'impression) 🔥 */}
            {listeInstitutsUniques.size > 0 && (
              <div className="flex items-center gap-2 print:hidden">
                <Filter className="w-4 h-4 text-muted-foreground" />
                <Select value={filterInstitut} onValueChange={setFilterInstitut}>
                  <SelectTrigger className="w-[200px] bg-background/50 rounded-xl h-10 border-border/60 font-medium">
                  ...
                  <SelectContent className="rounded-xl">
                    ...
                    {Array.from(listeInstitutsUniques).map(inst => (
            ...
            <Table className="print:text-sm">
              <TableHeader className="bg-muted/30 print:bg-transparent">
                <TableRow className="border-border/40 print:border-b-2 print:border-black">
                  <TableHead className="font-bold px-6 py-4 print:px-2 text-foreground">Matière</TableHead>
                  <TableHead className="text-foreground print:px-2">Classe & Institut</TableHead>
                  <TableHead className="text-foreground print:px-2">Charge</TableHead>
                  <TableHead className="text-foreground print:px-2">Semestre</TableHead>
              ...
              <TableBody className="divide-y divide-border/40 print:divide-border">
                {filteredEnseignements.length === 0 ? (
                  ...
                  filteredEnseignements.map((ens, i) => {
                    ...
                    return (
                      <TableRow key={ens.id} className={`hover:bg-muted/20 border-border/40 print:break-inside-avoid ${!isValide ? 'opacity-70 print:opacity-100' : ''}`}>
                        <TableCell className="px-6 print:px-2 py-4">
                        ...
                        <TableCell className="print:px-2">
                          <span className="font-medium text-muted-foreground">{ens.course.matiere.semestre}</span>
                        </TableCell>
